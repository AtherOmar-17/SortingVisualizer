...
namespace QuickSDL {
	...
	// Interpolates linearly between the two given vectors
	// time - between 0.0 and 1.0
	//		if time <= 0.0 start is returned
	//		if time >= 1.0 end is returned
	...
	struct BezierCurve {
		...
		Vector2 CalculateCurvePoint(float t) {
			...
			Vector2 retVal = (uuu*p0) + (3 * uu*t*p1) + (3 * u*tt*p2) + (ttt*p3);
			retVal.x = (float)round(retVal.x);
			retVal.y = (float)round(retVal.y);
			return retVal;
		}
	};
}
